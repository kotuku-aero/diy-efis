...
| Parameter | Type | Direction | Description |
|-----------|------|-----------|-------------|
| `handle` | `handle_t*` | inout | on the first call this is nullptr, it must then be returned on all subsequent calls. close_handle must be called when the enumeration is over |
