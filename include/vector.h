...
static inline result_t _ty##s_ensure_space( _ty##s_t *vec, uint16_t length)    \
  ...
  if (vec->buffer != nullptr) \
    { \
    if((vec->buffer = vector_realloc(vec->buffer, array_size * sizeof( _ty )))== nullptr)  \
      return e_no_space; \
    } \
  else \
    { \
    if((vec->buffer = vector_malloc(array_size * sizeof( _ty )))== nullptr) \
      return e_no_space; \
    } \
  vec->end = array_size;    \
  return s_ok;    \
...
static inline result_t _ty##s_create( _ty##s_t **vec)    \
  {    \
  if((vec = vector_malloc(sizeof( _ty##s_t )))== nullptr) \
    return e_no_space; \
  if (*vec == 0)    \
    return e_no_space;    \
...
static inline result_t _ty##s_close( _ty##s_t *vec) \
  { if (vec->buffer != 0) vector_free(vec->buffer); vector_free(vec); return s_ok; }    \
