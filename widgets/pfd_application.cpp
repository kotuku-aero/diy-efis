...
kotuku::pfd_application_t::pfd_application_t(layout_window_t *root_window)
: _root_window(root_window)
  ...
  if(succeeded(the_hal()->get_config_value("diy-efis", "node-id", node_id)))
    _node_id = (uint8_t) node_id;
  }

kotuku::pfd_application_t::~pfd_application_t()
  {
  }

uint8_t kotuku::pfd_application_t::node_id() const
  {
  return _node_id;
  }
...
void kotuku::display_roller(kotuku::canvas_t &cv,
  ...
  kotuku::point_t pt(bounds.right -(digits == 1 ? size_medium.dx >>= 1 : size_medium.dx), bounds.top);
  ...
  pt.y += gdi_dim_t((gdi_dim_t(value) % 10) *(size_medium.dy / 10.0));
