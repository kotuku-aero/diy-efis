...
namespace CanFly.Tools.MetadataProcessor
{
  ...
  /// this collected list into target assembly inFramework format.
  /// </summary>
  public sealed class TypeDefinitionTable : ReferenceTableBase<TypeDefinition>
  {
    /// <summary>
    ...
    /// <summary>
    /// Creates new instance of <see cref="TypeDefinitionTable"/> object.
    /// </summary>
    /// <param name="items">List of types definitins in Mono.Cecil format.</param>
    /// <param name="context">
    /// Assembly tables context - contains all tables used for building target assembly.
    /// </param>
    public TypeDefinitionTable(IEnumerable<TypeDefinition> items, TablesContext context)
    ...
    public bool TryGetTypeReferenceId(TypeDefinition typeDefinition, out ushort referenceId)
    {
    ...
    public IEnumerable<Tuple<uint, uint>> GetByteCodeOffsets(uint clrMethodToken)
    {
      return _byteCodeOffsets[clrMethodToken];
    }

    /// <inheritdoc/>
    protected override void WriteSingleItem(CLRBinaryWriter writer, TypeDefinition item)
    {
      ...
      using (var stream = new MemoryStream(6))
      {
        ...
        if (_context.MinimizeComplete)
        {
          ...
          foreach (TypeDefinition nestedType in item.NestedTypes)
          {
      ...
      // write flags
      writer.WriteUInt16((ushort)GetFlags(item, _context.MethodDefinitionTable));
    }

    private void WriteClassFields(IList<FieldDefinition> fieldsList, CLRBinaryWriter writer)
    {
      ushort firstStaticFieldId = 0;
      var staticFieldsCount = 0;
      foreach (FieldDefinition field in fieldsList.Where(item => item.IsStatic))
      {
        ...
        CLRBinaryWriter writer)
    {
      ...
      foreach (MethodDefinition method in methods.Where(item => item.IsVirtual))
      {
      ...
      foreach (MethodDefinition method in methods.Where(item => !(item.IsVirtual || item.IsStatic)))
      {
        firstMethodId = Math.Min(firstMethodId, _context.ByteCodeTable.GetMethodId(method));
        CreateMethodSignatures(method);
      ...
      foreach (MethodDefinition method in methods.Where(item => item.IsStatic))
      {
        firstMethodId = Math.Min(firstMethodId, _context.ByteCodeTable.GetMethodId(method));
        CreateMethodSignatures(method);
        ++staticMethodsCount;
    ...
    private void CreateMethodSignatures(MethodDefinition method)
    {
    ...
    private ushort GetTypeReferenceOrDefinitionId(TypeReference typeReference)
    {
      ushort referenceId;
    ...
    /// </remarks>
    internal static nanoTypeDefinitionFlags GetFlags(TypeDefinition definition, MethodDefinitionTable methodDefinitioTable = null)
    {
