...
namespace CanFly.Tools.MetadataProcessor
{
  ...
  public sealed class CLRAssemblyDefinition
  {
    ...
    /// </summary>
    private const string c_NFAssemblyMarker_v1 = "CANFLY";
        ...
        bool isPreAllocationCall)
    {
      ...
      // string table offset
      writer.WriteUInt16(isPreAllocationCall ? (ushort)0x0000 : _context.StringTable.GetOrCreateStringId(_context.AssemblyDefinition.Name.Name));
      ...
      else
      {
        ...
        var assemblyCrc32 = ComputeCrc32(writer.BaseStream, _paddingsOffset, writer.BaseStream.Length - _paddingsOffset);
        ...
        long tablePadding)
    {
      ...
      _tablesOffset += sizeof(uint);

      writer.BaseStream.Seek(_paddingsOffset, SeekOrigin.Begin);
      writer.WriteUInt32((uint)tablePadding);
      _paddingsOffset += sizeof(uint);
