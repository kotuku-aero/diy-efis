...
void DefinitionsGenerator::generate_id_definitions(std::ofstream &out)
  {
  //pugi::xpath_node_set ids =
  //    parser_.document().document_element().select_nodes("//id");
  //int position = 1;
...
void DefinitionsGenerator::generate_callback_methods(std::ofstream &out)
  ...
  pugi::xpath_node_set methods =
      doc.select_nodes("//*[./@on-message[normalize-space() != '']]");
  ...
  for (auto it = methods.begin(); it != methods.end(); ++it)
    ...
    if (!method.empty() && methods_declared.insert(method).second)
      ...
      out << "extern void " << method
          << "(handle_t canvas, const rect_t *wnd_rect, const canmsg_t* msg, "
             "void* wnddata);\n";
      }
    }
  ...
  methods =
      doc.select_nodes("//*[./@on-item-selected[normalize-space() != '']]");
  for (auto it = methods.begin(); it != methods.end(); ++it)
    {
    std::string method =
        get_named_attribute(it->node(), "on-item-selected").value();
    if (!method.empty() && methods_declared.insert(method).second)
      {
      out << "extern result_t " << method
          << "(widget_t* widget, const event_args_t *args);\n";
