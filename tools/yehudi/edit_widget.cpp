...
void code_generator::create_edit_widget(std::ofstream &out,
                                        const pugi::xml_node &node,
                                        const char *prefix,
                                        const char *name_prefix)
  {
  out << "static edit_widget_t " << name_prefix
      << get_named_attribute(node, "id").value() << " = {" << std::endl;

  // out << ".base.base.base.base.type = &edit_widget_type," << std::endl;
  ...
  attr = get_named_attribute(node, "wrap").value();
  if (attr == "true" || attr == "1")
    style += "eto_wrap |";
