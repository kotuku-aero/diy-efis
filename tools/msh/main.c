...
static inline win_stream_handle_t *as_stream(stream_handle_t *hndl)
  {
  return (win_stream_handle_t *) hndl;
  }

result_t stream_eof(stream_handle_t *hndl)
  {
  win_stream_handle_t *stream = as_stream(hndl);

  return stream->pos >= stream->size ? s_ok : s_false;
  }

result_t stream_read(stream_handle_t *hndl, void *buffer, uint16_t size, uint16_t *read)
  {
  win_stream_handle_t *stream = as_stream(hndl);

  SetFilePointer(stream->file, stream->pos, 0, FILE_BEGIN);

  DWORD bytesRead = 0;

  if(!ReadFile(stream->file, buffer, size, &bytesRead, NULL))
    return e_unexpected;

  stream->pos += (uint16_t) bytesRead;
...
result_t stream_write(stream_handle_t *hndl, const void *buffer, uint16_t size)
  {
  return e_invalid_operation;
  }

result_t stream_getpos(stream_handle_t *hndl, uint16_t *pos)
  {
  win_stream_handle_t *stream = as_stream(hndl);
...
result_t stream_setpos(stream_handle_t *hndl, uint16_t pos)
  {
  win_stream_handle_t *stream = as_stream(hndl);
...
result_t stream_length(stream_handle_t *hndl, uint16_t *length)
  {
  win_stream_handle_t *stream = as_stream(hndl);
...
result_t stream_truncate(stream_handle_t *hndl, uint16_t length)
  {
  return e_invalid_operation;
  }

result_t stream_close(stream_handle_t *hndl)
  {
  win_stream_handle_t *stream = as_stream(hndl);
...
result_t stream_delete(stream_handle_t *hndl)
  {
  return e_invalid_operation;
  }

result_t stream_path(stream_handle_t *hndl, bool full_path, uint16_t len, char *path)
  {
  return e_not_implemented;
  }

// specific embedded resources
result_t manifest_open(const char *path, handle_t *stream)
  {
  if(path == 0 || stream == 0)
    return e_bad_parameter;

  HANDLE hndl = CreateFile(path, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
  if(hndl == NULL)
    return e_not_found;

  DWORD fileSize;
  GetFileSize(hndl, &fileSize);
  ...
  win_stream_handle_t *resource_stream = (win_stream_handle_t *) neutron_malloc(sizeof(win_stream_handle_t));
  resource_stream->file = hndl;
  resource_stream->stream.stream_close = stream_close;
  resource_stream->stream.stream_delete = stream_delete;
  resource_stream->stream.stream_eof = stream_eof;
  resource_stream->stream.stream_getpos = stream_getpos;
  resource_stream->stream.stream_length = stream_length;
  resource_stream->stream.stream_path = stream_path;
  resource_stream->stream.stream_read = stream_read;
  resource_stream->stream.stream_setpos = stream_setpos;
  resource_stream->stream.stream_truncate = stream_truncate;
  resource_stream->stream.stream_write = stream_write;
  resource_stream->stream.version = sizeof(win_stream_handle_t);
  resource_stream->pos = 0;
  resource_stream->size = (uint16_t) fileSize;
