...
static void re_string_construct_common (const char *str, int len, re_string_t *pstr, RE_TRANSLATE_TYPE trans, int icase, const re_dfa_t *dfa) ;
static re_dfastate_t *create_ci_newstate (const re_dfa_t *dfa, const re_node_set *nodes, unsigned int hash) ;
static re_dfastate_t *create_cd_newstate (const re_dfa_t *dfa, const re_node_set *nodes, unsigned int context, unsigned int hash) ;

...
static reg_errcode_t  re_string_allocate (re_string_t *pstr, const char *str, int len, int init_len, RE_TRANSLATE_TYPE trans, int icase, const re_dfa_t *dfa)
  {
  reg_errcode_t ret;
  int init_buf_len;
...
weak_alias (__re_compile_fastmap, re_compile_fastmap)
  ...
  inline void re_set_fastmap (char *fastmap, int icase, int ch)
  {
  fastmap[ch] = 1;
  if (icase)
...
# if SHLIB_COMPAT (libc, GLIBC_2_0, GLIBC_2_3_4)
  ...
  int
    ...
    __compat_regexec (const regex_t * preg,
...
# endif
    ...
    static reg_errcode_t re_search_internal (const regex_t *preg, const char *string,
      int length, int start, int range, int stop, size_t nmatch, regmatch_t *pmatch,
      int eflags)
      {
      reg_errcode_t err;
...
free_return:
    ...
    inline re_dfastate_t *acquire_init_state_context (reg_errcode_t *err, const re_match_context_t *mctx, int idx)
      {
      const re_dfa_t *const dfa = mctx->dfa;
      if (dfa->init_state->has_constraint)
        {
