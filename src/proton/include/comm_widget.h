...
typedef struct _comm_window_t {
  ...
  bool rx_active;
  char comm_freq_area[CFG_NAME_MAX];

  uint16_t stby_freq;
  char stby_freq_area[CFG_NAME_MAX];
...
extern result_t create_comm_window(handle_t parent, uint32_t flags, comm_window_t* wnd, handle_t* hndl);
