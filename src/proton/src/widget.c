...
// for testing purposes, we don't want the timer to be used
#ifdef _WIN32
#define _NO_TIMER
#endif

void on_paint_widget_background(handle_t canvas, const rect_t* wnd_rect, const canmsg_t* msg, widget_t* widget)
  {
  extent_t ex;
  ...
  if (style & FILL_BACKGROUND)
    ...
    if (widget->background_gutter != 0)
      {
      // left gutter
      rectangle(canvas, wnd_rect, color_hollow, widget->gutter_color,
        rect_create_ex(0, 0, widget->background_gutter, ex.dy, &rect));
      // right gutter
      rectangle(canvas, wnd_rect, color_hollow, widget->gutter_color,
        rect_create_ex(ex.dx - widget->background_gutter, 0, widget->background_gutter, ex.dy, &rect));
      // top gutter
      rectangle(canvas, wnd_rect, color_hollow, widget->gutter_color,
        rect_create_ex(widget->background_gutter, 0,
          ex.dx - (widget->background_gutter << 1), widget->background_gutter, &rect));
      // bottom gutter
      rectangle(canvas, wnd_rect, color_hollow, widget->gutter_color,
        rect_create_ex(widget->background_gutter, ex.dy - widget->background_gutter,
          ex.dx - (widget->background_gutter << 1), widget->background_gutter, &rect));
      }

    rectangle(canvas, wnd_rect, 0, widget->background_color,
      rect_create(widget->background_gutter, widget->background_gutter,
        ex.dx - widget->background_gutter, ex.dy - widget->background_gutter, &rect));
...
result_t widget_wndproc(handle_t hwnd, const canmsg_t* msg, void* wnddata)
  ...
  if (widget->sensor_id == id)
    ...
    if (succeeded(get_param_bool(msg, &flag)))
      {
      changed = widget->is_alarm != flag;
      widget->is_alarm = flag;
      }
    }
  else if (id == id_paint)
    on_paint_widget(hwnd, msg, wnddata);
  else if (widget->on_message != 0)
    return widget->on_message(hwnd, msg, wnddata);
...
result_t create_widget(handle_t parent, uint32_t flags, wndproc_fn cb, widget_t* widget, handle_t* hwnd)
  {
  if (widget == nullptr || cb == nullptr)
    return e_bad_parameter;

  result_t result;


  if (failed(result = window_create(parent, &widget->rect, cb, widget, flags, hwnd)))
    return result;
...
result_t on_paint_widget(handle_t hwnd, const canmsg_t* msg, widget_t* widget)
  ...
  if (widget->is_alarm)
    ...
    // draw a border
    point_t pts[5];
    pts[0].x = wnd_rect.left + 1; pts[0].y = wnd_rect.top + 1;
    pts[1].x = wnd_rect.right - 1; pts[1].y = wnd_rect.top + 1;
    pts[2].x = wnd_rect.right - 1; pts[2].y = wnd_rect.bottom - 1;
    pts[3].x = wnd_rect.left + 1; pts[3].y = wnd_rect.bottom - 1;
    pts[4].x = wnd_rect.left + 1; pts[4].y = wnd_rect.top + 1;

    polyline(canvas, &wnd_rect, widget->alarm_color, 5, pts);
    }
...
//  const handle_t  *old_font = cv.font(&arial_12_font);
  ...
  sprintf(str, large_fmt, large_value);
