...
static int trex_newnode(TRex *exp, TRexNodeType type)
{
	...
	if(exp->_nallocated < (exp->_nsize + 1)) {
		...
		exp->_nallocated *= 2;
    exp->_nodes = realloc(exp->_nodes, exp->_nallocated * sizeof(TRexNode));
	}
...

	{
		...
		switch(*exp->_p){
			...
			case '{':
				...
				p0 = (uint16_t)trex_parsenumber(exp);
				/*******************************/
				switch(*exp->_p) {
			case '}':
				p1 = p0; exp->_p++;
			...
			case ',':
				...
				p1 = 0xFFFF;
				if(isdigit(*exp->_p)){
					p1 = (uint16_t)trex_parsenumber(exp);
				}
				trex_expect(exp,'}');
				break;
			default:
...
TRex *trex_compile(const TRexChar *pattern,const TRexChar **error)
{
	...
	exp->_nodes = malloc(exp->_nallocated * sizeof(TRexNode));
	exp->_nsize = 0;
	exp->_matches = 0;
	exp->_nsubexpr = 0;
...
#endif
		exp->_matches = malloc(exp->_nsubexpr * sizeof(TRexMatch));
