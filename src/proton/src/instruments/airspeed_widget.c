#include "../../include/airspeed_widget.h"
...
static void on_paint(handle_t canvas, const rect_t* wnd_rect, const canmsg_t* msg, void* wnddata)
  ...
  //point_t pt = { 0, 0 };
  //bit_blt(canvas, wnd_rect, wnd_rect, wnd->background_canvas, wnd_rect, &pt, src_alpha_blend);
  ...
  gdi_dim_t vne_pixels = (gdi_dim_t)((top_asi - wnd->converter->convert(mfd.aircraft.vne)) * wnd->pixels_per_unit);
  gdi_dim_t vno_pixels = (gdi_dim_t)((top_asi - wnd->converter->convert(mfd.aircraft.vno)) * wnd->pixels_per_unit);
  gdi_dim_t va_pixels = (gdi_dim_t)((top_asi - wnd->converter->convert(mfd.aircraft.va)) * wnd->pixels_per_unit);
  gdi_dim_t vfe_pixels = (gdi_dim_t)((top_asi - wnd->converter->convert(mfd.aircraft.vfe)) * wnd->pixels_per_unit);
  gdi_dim_t vs0_pixels = (gdi_dim_t)((top_asi - wnd->converter->convert(mfd.aircraft.vs0)) * wnd->pixels_per_unit);      // stall flaps extended
  gdi_dim_t vs1_pixels = (gdi_dim_t)((top_asi - wnd->converter->convert(mfd.aircraft.vs1)) * wnd->pixels_per_unit);      // stall flaps up
  gdi_dim_t vx_pixels = (gdi_dim_t)((top_asi - wnd->converter->convert(mfd.aircraft.vx)) * wnd->pixels_per_unit);        // best angle of climb
  gdi_dim_t vy_pixels = (gdi_dim_t)((top_asi - wnd->converter->convert(mfd.aircraft.vy)) * wnd->pixels_per_unit);        // best rate of climb
...
static result_t airspeed_wndproc(handle_t hwnd, const canmsg_t *msg, void *wnddata)
  ...
  if (get_can_id(msg) == id_indicated_airspeed)
    ...
    float airspeed = wnd->converter->convert(v);
...
result_t create_airspeed_widget(handle_t parent, uint32_t flags, airspeed_widget_t* wnd, handle_t* out)
  {
  result_t result;
  handle_t hndl;
  if (failed(result = create_widget(parent, flags, airspeed_wndproc, &wnd->base, &hndl)))
    return result;
