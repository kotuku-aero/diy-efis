#include "../../include/gauge_widget.h"
...
static void draw_pointer2(handle_t canvas, gauge_widget_t* wnd, const rect_t* wnd_rect, color_t pen, int rotation);
static void draw_pointer3(handle_t canvas, gauge_widget_t* wnd, const rect_t* wnd_rect, color_t pen, int rotation);
static void draw_sweep(handle_t canvas, gauge_widget_t* wnd, const rect_t* wnd_rect, color_t pen, color_t fill,
                       int rotation);
...
static inline float to_display_int32_to_float(gauge_widget_t* wnd, int32_t value)
  {
  return wnd->converter != nullptr ? wnd->converter->convert(value) : value;
  }

static inline float to_display_float_to_float(gauge_widget_t* wnd, float value)
  {
  return wnd->converter != nullptr ? wnd->converter->convert(value) : value;
  }

void on_paint_gauge_background(handle_t canvas, const rect_t* wnd_rect, const canmsg_t* msg, void* wnddata)
  {
...
void on_paint_gauge(handle_t canvas, const rect_t* wnd_rect, const canmsg_t* msg, void* wnddata)
  {
  gauge_widget_t* wnd = (gauge_widget_t*)wnddata;
  ...
  if (wnd->background_canvas == nullptr)
    ...
    (wnd->base.on_paint_background)(wnd->background_canvas, wnd_rect, msg, wnddata);
    }

  point_t pt = {0, 0};
  bit_blt(canvas, wnd_rect, wnd_rect, wnd->background_canvas, wnd_rect, &pt, src_copy);

  //(wnd->base.on_paint_background)(canvas, wnd_rect, msg, wnddata);
...
static void on_paint_background_hbar(handle_t canvas, gauge_widget_t* wnd, const rect_t* wnd_rect)
  ...
  float range = steps[num_steps - 1].value - steps[0].value;

  int32_t wnd_width = rect_width(wnd_rect);
  float pixels_per_unit = ((float)wnd_width - 16) / (range);
  ...
  for (uint16_t step = 1; step < num_steps; step++)
    {
    float relative_value = steps[step].value - steps[step - 1].value;
...
static void on_paint_hbar(handle_t canvas, gauge_widget_t* wnd, const rect_t* wnd_rect)
  ...
  float value = to_display_float_to_float(wnd, wnd->value);
  ...
  if (wnd->draw_value && wnd->value_font != 0)
    ...
    char str[32];
    if (wnd->converter != nullptr)
      {
      if (wnd->converter->is_float)
        sprintf(str, wnd->converter->format, value);
      else
        sprintf(str, wnd->converter->format, (int)(value));
      }
    else
    ...
    if (wnd->draw_value_box)
      ...
      rectangle(canvas, wnd_rect, color_grey, color_hollow, &wnd->value_rect);
...
static void on_paint_background_bar(handle_t canvas, gauge_widget_t* wnd, const rect_t* wnd_rect)
  ...
  if (num_ticks != 0)
    ...
    for (i = 0; i < num_ticks; i++)
      ...
      color_t color = last_tick->color;
      if (color == color_hollow)
        color = wnd->base.name_color;

      draw_text(canvas, wnd_rect, wnd->font,
                color, wnd->base.background_color,
...
static void on_paint_background_dial(handle_t canvas, gauge_widget_t* wnd, const rect_t* wnd_rect)
  ...
  if (num_ticks > 0)
    ...
    int32_t line_start = wnd->gauge_radii - wnd->arc_width;
    int arc_start;
    ...
    for (i = 0; i < num_ticks; i++)
      ...
      int32_t line_end = line_start - (tick->length == 0 ? 5 : tick->length);
      ...
      if (tick->text != 0 && wnd->font != 0)
        ...
        draw_text(canvas, wnd_rect, wnd->font, color, wnd->base.background_color,
...
static void on_paint_dial(handle_t canvas, gauge_widget_t* wnd, const rect_t* wnd_rect)
  {
  float value = wnd->converter != nullptr ? wnd->converter->convert(wnd->value) : wnd->value;
  float min_value;
  float max_value;
  ...
  case gs_pointer_minmax:
    min_value = wnd->converter != nullptr ? wnd->converter->convert(wnd->min_value) : wnd->min_value;
    max_value = wnd->converter != nullptr ? wnd->converter->convert(wnd->max_value) : wnd->max_value;
    draw_point(canvas, wnd, wnd_rect,
               calculate_pen(wnd, min_value),
               calculate_rotation(wnd, min_value));
    draw_point(canvas, wnd, wnd_rect,
               calculate_pen(wnd, max_value),
               calculate_rotation(wnd, max_value));
    break;
  case gs_pointer:
    if (wnd->incremental)
      {
      float incr_value = (float)(((int)value) % 1000);

      draw_pointer(canvas, wnd, wnd_rect,
                   calculate_pen(wnd, incr_value),
                   calculate_rotation(wnd, incr_value) + 90);

      incr_value = wnd->value / 10;
      draw_pointer2(canvas, wnd, wnd_rect,
                    calculate_pen(wnd, incr_value),
                    calculate_rotation(wnd, incr_value) + 90);

      incr_value = wnd->value / 100;
      draw_pointer3(canvas, wnd, wnd_rect,
                    calculate_pen(wnd, incr_value),
                    calculate_rotation(wnd, incr_value) + 90);
      }
    else
      draw_pointer(canvas, wnd, wnd_rect,
                   calculate_pen(wnd, value),
                   calculate_rotation(wnd, value) + 90);
    break;
  case gs_sweep:
  ...
  if (wnd->draw_value && wnd->value_font != 0)
    ...
    if (wnd->draw_value_box)
      {
      // draw a rectangle around the text
      rectangle(canvas, wnd_rect, color_grey, color_black, &wnd->value_rect);
...
static void draw_pointer(handle_t canvas,
  ...
  point_t pts[5] =
    {
    {wnd->center.x + 4, wnd->center.y - 20},
    {wnd->center.x + 2, wnd->center.y - wnd->gauge_radii + 18},
    {wnd->center.x - 2, wnd->center.y - wnd->gauge_radii + 18},
    {wnd->center.x - 4, wnd->center.y - 20},
    };
  ...
  rotate_point(&wnd->center, rotation, &pts[3]);
  point_copy(&pts[0], &pts[4]);

  polygon(canvas, wnd_rect, outline_pen, outline_pen, 4, pts);
  }

// draw a second pointer on the gauge

static void draw_pointer2(handle_t canvas,
                          gauge_widget_t* wnd,
                          const rect_t* wnd_rect,
                          color_t outline_pen,
                          int rotation)
  ...
  point_t pts[6] =
    {
    {wnd->center.x + 4, wnd->center.y - 20},
    {wnd->center.x + 9, wnd->center.y - wnd->gauge_radii + 55},
    {wnd->center.x, wnd->center.y - wnd->gauge_radii + 30},
    {wnd->center.x - 9, wnd->center.y - wnd->gauge_radii + 55},
    {wnd->center.x - 4, wnd->center.y - 20},
    };
  ...
  rotate_point(&wnd->center, rotation, &pts[2]);
  rotate_point(&wnd->center, rotation, &pts[3]);
  rotate_point(&wnd->center, rotation, &pts[4]);
  point_copy(&pts[0], &pts[5]);

  polygon(canvas, wnd_rect, outline_pen, outline_pen, 6, pts);
  }

// draw a third pointer on the gauge

static void draw_pointer3(handle_t canvas,
                          gauge_widget_t* wnd,
                          const rect_t* wnd_rect,
                          color_t outline_pen,
                          int rotation)
  ...
  point_t pts[4] =
    {
    {wnd->center.x + 5, wnd->center.y - wnd->gauge_radii},
    {wnd->center.x, wnd->center.y - wnd->gauge_radii + 10},
    {wnd->center.x - 5, wnd->center.y - wnd->gauge_radii},
    };
  ...
  // draw the line
  pts[0].x = wnd->center.x;
  pts[0].y = wnd->center.y - wnd->gauge_radii + 10;
  pts[1].x = wnd->center.x;
  pts[1].y = wnd->center.y - 20;
...
static void draw_point(handle_t canvas,
  ...
  point_t pts[4] =
    {
    {wnd->center.x, (wnd->center.y - wnd->gauge_radii) + 9},
    {wnd->center.x - 5, (wnd->center.y - wnd->gauge_radii) - 6},
    {wnd->center.x + 5, (wnd->center.y - wnd->gauge_radii) - 6},
    {wnd->center.x, (wnd->center.y - wnd->gauge_radii) + 9}
    };
...
result_t gauge_wndproc(handle_t hwnd, const canmsg_t* msg, void* wnddata)
  ...
  if (wnd->reset_id == id)
    ...
    if (succeeded(get_param_float(msg, &float_value)))
      {
      // NOTE: Do not convert, as switching units only changes
      // the display values (ticks, and value box)
...
result_t create_gauge_widget(handle_t parent, uint32_t flags, gauge_widget_t* wnd, handle_t* out)
  {
  result_t result;
  handle_t hndl;
  if (failed(result = create_widget(parent, flags, gauge_wndproc, &wnd->base, &hndl)))
    return result;

  if (wnd->base.on_paint == nullptr)
    wnd->base.on_paint = on_paint_gauge;

  if (wnd->base.on_paint_background == nullptr)
    wnd->base.on_paint_background = on_paint_gauge_background;
