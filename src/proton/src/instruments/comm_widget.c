...
result_t create_comm_window(handle_t parent, uint32_t flags, comm_window_t* wnd, handle_t* out)
  {
  result_t result;
  handle_t hndl;
  if (failed(result = create_widget(parent, flags, comm_wndproc, &wnd->base, &hndl)))
    return result;
