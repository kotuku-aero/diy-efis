#include "../../include/edutemps_widget.h"
...
static void on_paint(handle_t canvas, const rect_t* wnd_rect, const canmsg_t* msg, void* wnddata)
  {
  edutemps_widget_t* wnd = (edutemps_widget_t*)wnddata;
  extent_t ex;
  ...
  if (wnd->background_canvas == nullptr)
    ...
    sprintf(val, "%d", wnd->cht_red_line);
    ...
    text_pt.y = wnd->red_line_pos - (text_ex.dy >> 1);

    draw_text(wnd->background_canvas, wnd_rect, wnd->font, color_scarlet, color_hollow, len, val, &text_pt, wnd_rect, eto_none, 0);

    // draw the EGT redline
    sprintf(val, "%d", wnd->egt_red_line);
    ...
    for (uint16_t i = 0; i < wnd->num_cylinders; i++)
      ...
      pts[0].x = pos - bar_center + 1; pts[0].y = wnd->red_line_pos;
      pts[1].x = pos - 1; pts[1].y = wnd->red_line_pos;
      polyline(wnd->background_canvas, wnd_rect, color_scarlet, 2, pts);
      pts[0].y++;
      pts[1].y++;
      polyline(wnd->background_canvas, wnd_rect, color_scarlet, 2, pts);
  ...
  uint16_t egt_min = wnd->egt_red_line;
  uint16_t egt_max = wnd->egt_min;
  uint16_t max_cyl = 7;
  uint16_t min_cyl = 7;
  for (uint16_t i = 0; i < wnd->num_cylinders; i++)
    {
    uint16_t value = (uint16_t) wnd->converter->convert(wnd->egt[i].value);
  ...
  for (uint16_t i = 0; i < wnd->num_cylinders; i++)
    ...
    if (i == max_cyl)
      ...
      rect_t egt_min_rect;
      rect_create(text_pt.x - 2, text_pt.y, text_pt.x + text_ex.dx, wnd_rect->bottom - 1, &egt_min_rect);
      rectangle(canvas, wnd_rect, color_green, color_hollow, &egt_min_rect);
      draw_text(canvas, wnd_rect, wnd->font, color_white, color_hollow, 1, cyl_numbers[i], &text_pt, wnd_rect, eto_none, 0);
      }
  ...
  for (uint16_t i = 0; i < wnd->num_cylinders; i++)
    ...
    if (bar->sensor_fail)
      ...
      draw_failed_sensor(canvas, wnd_rect, color_scarlet, &graph_rect);
      }
    else
      {
      float value = wnd->converter->convert(bar->value);
      sprintf(val, "%0.0f", value);
      len = (uint16_t)strlen(val);
      ...
      color_t color = (value >= wnd->cht_red_line) ? color_scarlet : bar->default_color;

      draw_text(canvas, wnd_rect, wnd->font,
        (bar->is_alarm) ? bar->alarm_color : color,
        color_hollow, len, val, &text_pt, wnd_rect, eto_none, 0);

      int32_t top_pix = calculate_height(value, wnd->red_line_pos, wnd->bar_top, wnd->bar_bottom,
        wnd->cht_red_line, wnd->cht_min);

      if (top_pix < wnd->bar_bottom)
        {

        rect_create(pos + 3, top_pix, pos + graph_width, wnd->bar_bottom, &graph_rect);

        draw_bar(canvas, wnd_rect, color_hollow, color, &graph_rect);
        }
      }
  ...
  for (uint16_t i = 0; i < wnd->num_cylinders; i++)
    ...
    if (bar->sensor_fail)
      ...
      color_t color = (value >= wnd->egt_red_line) ? color_scarlet : bar->default_color;

      text_pt.x = pos - 4 - (text_ex.dx >> 1);
      text_pt.y = wnd->egt_line;
      draw_text(canvas, wnd_rect, wnd->font, color, color_hollow, len, val, &text_pt, wnd_rect, eto_none, 0);
...
result_t create_edutemps_widget(handle_t parent, uint32_t flags, edutemps_widget_t* wnd, handle_t* out)
  {
  result_t result;
  handle_t hndl;
  if (failed(result = create_widget(parent, flags, edutemp_wndproc, &wnd->base, &hndl)))
    return result;
