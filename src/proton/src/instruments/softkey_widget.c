#include "../../include/softkey_widget.h"
...
result_t create_softkey_window(handle_t parent, uint32_t flags, softkey_window_t* wnd, handle_t* out)
  {
  result_t result;
  handle_t hndl;

  if (failed(result = create_widget(parent, flags, softkey_wndproc, &wnd->base, &hndl)))
    return result;
