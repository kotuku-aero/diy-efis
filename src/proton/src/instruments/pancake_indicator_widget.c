#include "../../include/pancake_indicator_widget.h"
...
static void on_paint(handle_t canvas, const rect_t* wnd_rect, const canmsg_t* msg, void* wnddata)
  ...
  // draw the name if enabled
  if ((wnd->base.style & DRAW_NAME) != 0)
    draw_text(canvas, wnd_rect, wnd->base.name_font, wnd->base.name_color, wnd->base.background_color, 0,
      wnd->base.name, &wnd->base.name_pt, wnd_rect, 0, 0);
  ...
  for (int i = 0; i < 9; i++)
    {
    bool fill_cell = wnd->bar_fill ? (i <= cell) : i == cell;

    rectangle(canvas, wnd_rect, wnd->outline_color,
      (fill_cell) ? wnd->indicator_color : color_black/*wnd->base.background_color*/,
...
static result_t pancake_wndproc(handle_t hwnd, const canmsg_t* msg, void* wnddata)
  {
  // TODO: min/max values are dependent on the selected ranges
  pancake_widget_t* wnd = (pancake_widget_t*)wnddata;
  ...
  if (wnd->can_id == id)
    ...
    if (wnd->converter != 0)
      value = (int16_t) (*wnd->converter->convert)(value);
...
result_t create_pancake_widget(handle_t parent, uint32_t flags, pancake_widget_t* wnd, handle_t* out)
  {
  result_t result;
  handle_t hndl;
  if (failed(result = create_widget(parent, flags, pancake_wndproc, &wnd->base, &hndl)))
    return result;
