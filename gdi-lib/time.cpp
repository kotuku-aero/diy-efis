...
kotuku::date_time_t::date_time_t(uint16_t year,
  ...
  {
  _t = the_hal()->mktime(year, month, day, hour, minute, second, milliseconds, nanoseconds);
  }
...
void kotuku::date_time_t::gmtime(uint16_t *year,
  ...
  the_hal()->gmtime(_t, year, reinterpret_cast<uint8_t *>(month), day, hour, minute, second, milliseconds, nanoseconds);
  }
