...
static result_t render_polygon(atomdb_viewport_t* viewport, const spatial_entity_id_t* id, handle_t canvas, const rect_t* clip_rect, const viewport_params_t* params)
  ...
  for(gdi_polyline_t* it = gdi_polylines_begin(&viewport->gdi_polylines); it != gdi_polylines_end(&viewport->gdi_polylines); it++)
    polygon(canvas, clip_rect, color_hollow, theme->land_color, points_count(&it->gdi_points), points_begin(&it->gdi_points));
...
static result_t render_polyline(atomdb_viewport_t* viewport, const spatial_entity_id_t* id, handle_t canvas, const rect_t* clip_rect, const viewport_params_t* params)
  ...
  for(gdi_polyline_t* it = gdi_polylines_begin(&viewport->gdi_polylines); it != gdi_polylines_end(&viewport->gdi_polylines); it++)
    polyline(canvas, clip_rect, theme->coastline_color, points_count(&it->gdi_points), points_begin(&it->gdi_points));

  return s_ok;
  }

result_t create_coastlines_viewport(handle_t container, const db_header_t* hdr, viewport_t** out)
  {
  result_t result;
