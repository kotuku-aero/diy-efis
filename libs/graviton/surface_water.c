...
static result_t render_polygon(atomdb_viewport_t* viewport, const spatial_entity_id_t* id, handle_t canvas, const rect_t* clip_rect, const viewport_params_t* params)
  ...
  for (gdi_polyline_t* it = gdi_polylines_begin(&viewport->gdi_polylines); it != gdi_polylines_end(&viewport->gdi_polylines); it++)
    polygon(canvas, clip_rect, color_hollow, theme->water_color, points_count(&it->gdi_points), points_begin(&it->gdi_points));

  return s_ok;
  }

static result_t render_airspace(atomdb_viewport_t* viewport, const spatial_entity_id_t* id, handle_t canvas, const rect_t* clip_rect, const viewport_params_t* params)
  {
  const map_theme_t* theme = params->map->is_night ? &params->map->night_theme : &params->map->day_theme;

  for (gdi_polyline_t* it = gdi_polylines_begin(&viewport->gdi_polylines); it != gdi_polylines_end(&viewport->gdi_polylines); it++)
    polyline(canvas, clip_rect, theme->water_color, points_count(&it->gdi_points), points_begin(&it->gdi_points));
