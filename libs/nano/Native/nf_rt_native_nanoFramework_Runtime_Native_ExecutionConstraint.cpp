...
HRESULT Library_nf_rt_native_nanoFramework_Runtime_Native_ExecutionConstraint::Install___STATIC__VOID__I4__I4(CLR_RT_StackFrame& stack)
  {
  ...
  if (sth->Next())
    ...
    NANOCLR_CHECK_HRESULT(CLR_RT_SubThread::CreateInstance(stack.m_owningThread, caller, 0, sthNew));

    sth = sthNew;
    }

  NANOCLR_CHECK_HRESULT(g_CLR_RT_ExecutionEngine.InitTimeout(sth->m_timeConstraint, timeout));
