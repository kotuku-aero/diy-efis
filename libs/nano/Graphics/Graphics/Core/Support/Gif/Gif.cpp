...
HRESULT CLR_GFX_Bitmap::CreateInstanceGif(CLR_RT_HeapBlock& ref, const CLR_UINT8* data, const CLR_UINT32 size)
  ...
  decoder = (GifDecoder*)CLR_RT_Memory::Allocate(sizeof(GifDecoder));
  CHECK_ALLOCATION(decoder);

  // Initialize the decompression engine
  NANOCLR_CHECK_HRESULT(decoder->GifInitDecompress(data, size));

  // At this point decoder would have all the correct output dimension info, so we initialized the
  // CLR_GFX_BitmapDescription with that information, note that we currently only support decompression into the
  // native bpp
  if (bm.BitmapDescription_Initialize(
    decoder->header.LogicScreenWidth,
    decoder->header.LogicScreenHeight,
