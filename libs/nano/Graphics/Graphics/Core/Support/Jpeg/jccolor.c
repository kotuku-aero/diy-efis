...
typedef struct {
  ...
  CLR_INT32* rgb_ycc_tab;		/* => table for RGB to YCbCr conversion */
  } my_color_converter;
...
#define CBCR_OFFSET	((CLR_INT32) CENTERJSAMPLE << SCALEBITS)
#define ONE_HALF	((CLR_INT32) 1 << (SCALEBITS-1))
#define FIX(x)		((CLR_INT32) ((x) * (1L<<SCALEBITS) + 0.5))
...
rgb_ycc_start(j_compress_ptr cinfo)
  ...
  CLR_INT32* rgb_ycc_tab;
  CLR_INT32 i;

  /* Allocate and fill in the conversion tables. */
  cconvert->rgb_ycc_tab = rgb_ycc_tab = (CLR_INT32*)
    (*cinfo->mem->alloc_small) ((j_common_ptr)cinfo, JPOOL_IMAGE,
      (TABLE_SIZE * SIZEOF(CLR_INT32)));
...
rgb_ycc_convert(j_compress_ptr cinfo,
  ...
  my_cconvert_ptr cconvert = (my_cconvert_ptr)cinfo->cconvert;
  register int r, g, b;
  register CLR_INT32* ctab = cconvert->rgb_ycc_tab;
