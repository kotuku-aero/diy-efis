...
static result_t as_tt_font(font_t *font, tt_font_t **tt_font)
  {
  if(font == 0 || tt_font == 0 || font->version != sizeof(tt_font_t))
    return e_bad_parameter;

  *tt_font = (tt_font_t *)font;
  return s_ok;
  }
...
result_t find_glyph_index(const true_type_font_t *info, uint16_t unicode_codepoint, uint16_t *glyph_offset)
  {
  ...
  *glyph_offset = 0;
  ...
  static double scale_for_pixel_height(const true_type_font_t *info, double height)
    {
...
static result_t get_glyph_shape(const true_type_font_t *font, uint16_t glyph_index, glyph_shape_t **hshape)
  {
  int num_vertices = 0;
  uint16_t offset = 0;
  uint16_t end_offset;
...
#define BITMAP_ANTI_ALIAS_SCALE 5
#define BITMAP_ANTI_ALIAS_OFFSET 3
...
static uint8_t pel_weights[16] = { 192, 128, 128, 64, 64, 32, 32, 16, 16, 8, 8, 4, 4, 2, 2, 1 };
...
static void fc_fast_line(canvas_t *hndl, const point_t *p1, const point_t *p2, color_t fill_color)
  ...
  gdi_dim_t x;
  // calculate the distancs
  gdi_dim_t y_pel = p1->y / BITMAP_ANTI_ALIAS_SCALE;

  gdi_dim_t x_pel = p1->x / BITMAP_ANTI_ALIAS_SCALE;
  gdi_dim_t x_incr = BITMAP_ANTI_ALIAS_SCALE - (p1->x % BITMAP_ANTI_ALIAS_SCALE);
  ...
  for (x = p1->x; x < p2->x; x++)
    ...
    uint16_t value = canvas->glyph->bitmap.pixels[x_pel + (y_pel * canvas->glyph->bitmap.width)];
    value += pel_weights[(value & 0xf0) >> 4]; // BITMAP_PEL_WEIGHT;
    if(value > 255)
      value = 255;

    canvas->glyph->bitmap.pixels[x_pel + (y_pel * canvas->glyph->bitmap.width)] = value;
    x_incr--;
    if(x_incr == 0)
      {
      x_incr = BITMAP_ANTI_ALIAS_SCALE;
      x_pel++;
      }
    }
  }
...
result_t ensure_glyph(sized_font_t *font_info, char ch, const glyph_t **gp)
  ...
  true_type_font_t *fontinfo = &font_info->fontrec->info;
...
result_t init_font(true_type_font_t *info, handle_t stream)
  {
  result_t result;
  uint16_t cmap;
  int32_t i;
...
result_t create_tt_font(font_t **font)
  {
  if(font == 0)
    return e_bad_parameter;

  *font = (font_t *)neutron_malloc(sizeof(tt_font_t));
  memset(*font, 0, sizeof(tt_font_t));
  (*font)->version = sizeof(tt_font_t);

  return s_ok;
  }

static result_t new_tt_font(font_t *font, uint16_t pixels, const char *hint, sized_font_t **glyphs)
  {
  result_t result;
  ...
  *glyphs = sf;
  ...
  double scale = scale_for_pixel_height(&sf, pixels);
  sf->vertical_height = (gdi_dim_t)floor(fabs(rect_height(&sf->clip_rect) * scale));
  sf->baseline = (gdi_dim_t)floor(sf->info.clip_rect.bottom * scale);
...
result_t init_tt_font(font_t *font, handle_t stream)
  {
  result_t result;
  uint16_t cmap;
  int32_t i;
  ...
  tt_font_t *info = 0;
  if(failed(result = as_tt_font(font, &info)))
    return result;
